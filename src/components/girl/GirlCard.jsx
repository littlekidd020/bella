...
const GirlCard = ({ model, isLarge = false }) => {
  ...
  return (
    <>
      ...
      <div className="relative w-full flex-1 min-h-0 rounded-2xl shadow-[0_12px_30px_rgba(45,24,34,0.22)] transition-all duration-500 group-hover:shadow-[0_20px_40px_rgba(248,74,136,0.32)]">
        ...
        <div className="absolute top-4 left-4 flex gap-2">
          {model.location && (
            <div className="flex items-center gap-1.5 bg-white/80 backdrop-blur-md border border-[#F84A88]/20 px-2.5 py-1 rounded-full shadow-sm">
              <MapPin size={10} className="text-[#F84A88]" />
              <span className="text-[8px] uppercase tracking-[0.2em] font-sans text-[#2D1822]/90 font-bold">
      ...
      <div className="pt-3 pb-1 px-1 flex flex-col gap-1.5 shrink-0 bg-transparent">
        <div className="flex items-center justify-between gap-2">
          <div className="flex items-center gap-2">
            <h3 className={`${isLarge ? 'text-xl md:text-2xl' : 'text-lg'} font-serif text-[#2D1822] tracking-wide italic font-bold leading-none`}>
              {model.name[lang] || model.name.en}
            </h3>
            <span className={`inline-flex items-center px-2 py-0.5 rounded-full text-[8px] font-bold font-sans tracking-[0.15em] bg-white/60 border border-[#F84A88]/15 backdrop-blur-sm ${isAvailable ? 'text-[#F84A88]' : 'text-[#2D1822]/50'}`}>
              {isAvailable ? t.girl.available : t.girl.unavailable}
            </span>
          </div>
          
          <div className="flex items-baseline gap-0.5 shrink-0">
            <p className={`${isLarge ? 'text-sm md:text-base' : 'text-sm'} font-serif text-[#F84A88] font-bold`}>
              {typeof model.stats.cup === 'string' ? model.stats.cup.replace("(Natural)", t.girl.natural) : model.stats.cup}
            </p>
            <p className="text-[7px] uppercase tracking-[0.1em] text-[#2D1822]/50 font-sans">{t.girl.breast}</p>
          </div>
        </div>

        <div className="flex items-center justify-between text-[10px] text-[#2D1822]/70 font-sans font-medium">
        ...
        {/* About Text Preview */}
        <div className="mt-1.5 border-l border-[#F84A88]/30 pl-3">
          <p className="text-[#2D1822]/70 text-[11px] font-sans leading-relaxed line-clamp-2">
