...
const Navbar = () => {
  ...
  return (
    <>
      <header className={`left-0 right-0 transition-all duration-300 sticky top-0 ${
        isOpen ? "z-[80]" : "z-[70]"
      } ${
        isScrolled && !isOpen
        ...
        <div className="mx-auto max-w-7xl px-4 md:px-10 py-3 md:py-4">
          <nav className="relative z-10 flex items-center justify-between w-full">
            <Link href="/" className={`${isScrolled || pathname !== "/" || isOpen ? "flex" : "hidden md:flex"} items-center group`}>
      ...
      <div className={`fixed inset-0 bg-[#FFE3EC]/95 backdrop-blur-3xl z-[75] transition-all duration-700 ease-[cubic-bezier(0.16,1,0.3,1)] flex flex-col items-center justify-center gap-8 ${isOpen ? 'opacity-100 pointer-events-auto translate-y-0' : 'opacity-0 pointer-events-none -translate-y-4'}`}>
        <div className="flex flex-col items-center gap-6 text-2xl font-serif italic tracking-[0.2em]">
          ...
          {/* Small heart decoration below navigation links */}
          <div className="flex justify-center mt-2 opacity-65">
             <span className="text-[#F05C88] text-sm">♥</span>
          </div>
        </div>
      </div>
    </>
