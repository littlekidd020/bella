...
const cbd = unique.filter(g => g.loc.toUpperCase().includes("CBD"));
const central = unique.filter(g => g.loc.toUpperCase().includes("CENTRAL") || g.loc === "中区");
const others = unique.filter(g => !g.loc.toUpperCase().includes("CBD") && !g.loc.toUpperCase().includes("CENTRAL") && g.loc !== "中区");
